  ...
  constructor(obj?: any) {
    if (obj != null) {
      if (typeof obj === 'object') {
        ...
        if (obj.status != null) this.status = obj.status;
        if (obj.accNo != null) this.accNo = obj.accNo;
        if (obj.type != null) this.type = obj.type;
        if (obj.reasonForChange != null)
          this.reasonForChange = obj.reasonForChange;
        if (obj.headerMsg != null) this.headerMsg = obj.headerMsg;
        if (obj.contentMsg != null) this.contentMsg = obj.contentMsg;
        if (obj.footerMsg != null) this.footerMsg = obj.footerMsg;
        if (obj.company != null) this.company = new EntityModel(obj.company);
        if (obj.client != null) this.client = new EntityModel(obj.client);
        if (obj.items != null)
          this.items = obj.items.map((x) => new ItemModel(x));
        if (obj.validFrom != null)
          this.validFrom = parseFirestoreDate(obj.validFrom);
        if (obj.validTo != null) this.validTo = parseFirestoreDate(obj.validTo);
        if (obj.supply != null) this.supply = obj.supply;
        ...
        if (obj.authorName != null) this.authorName = obj.authorName;
        if (obj.authorLocation != null)
          this.authorLocation = obj.authorLocation;
        if (obj.netAmount != null) this.netAmount = obj.netAmount;
        if (obj.discountAmount != null) this.discountAmount = obj.discountAmount;
