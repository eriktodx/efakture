...
function iso13616Prepare(iban: any) {
...
function iso7064Mod97_10(iban: any) {
  var remainder = iban,
    block;
...
function parseStructure(structure: any) {
  ...
  var regex = structure.match(/(.{3})/g).map(function (block: any) {
...
function electronicFormat(iban: any) {
...
function addSpecification(IBAN: any) {
...
addSpecification(
  new Specification("HU", 28, "F03F04F01F15F01", "HU42117730161111101800000000")
);
addSpecification(
...
addSpecification(
  new Specification("SC", 31, "U04F04F16U03", "SC18SSCB11010000000000001497USD")
);
addSpecification(
...
export function isValid(iban: any) {
  if (!isString(iban)) {
    return false;
  }
...
export function toBBAN(iban: any, separator: any) {
...
export function fromBBAN(countryCode: any, bban: any) {
...
export function isValidBBAN(countryCode: any, bban: any) {
  if (!isString(bban)) {
    return false;
  }
  var countryStructure = countries[countryCode];
...
export function printFormat(iban: any, separator: any) {
  if (typeof separator == "undefined") {
    separator = " ";
  }
